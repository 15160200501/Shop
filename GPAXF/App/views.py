...
from App.models import MainWhell, MainNav, MainMustBuy, MainShop, MainShow, FoodType, Goods, AXFUser, Cart, Order, \
    OrderGoods
...
from App.views_heiper import hash_str, send_email_activate, get_total_price
from GPAXF.settings import MEDIA_KEY_PREFIX, APP_PRIVATE_KEY, ALIPAY_PUBLIC_KEY, ALIPAY_APPID


def home(request):
    main_wheels = MainWhell.objects.all()
...
def market_with_params(request, typeid, childcid, order_rule, ORDER_PRICE=None):
...
def mine(request):
    ...
    if user_id:
        ...
        data['order_not_receive'] = Order.objects.filter(o_user=user). \
            filter(o_status__in=[ORDER_STATUS_NOT_RECEIVE, ORDER_STATUS_NOT_SEND]).count()
    ...
    if request.method == "GET":
        ...
        if users.exists():
            ...
            if check_password(password, user.u_password):
                ...
                print("密码错误")
                request.session['error_message'] = '输入密码错误'
                return redirect(reverse('axf:login'))
        print("用户不存在")
